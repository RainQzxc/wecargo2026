...
const PAYMENT_STATUS_LABELS: Record<string, string> = {
  ...
  UNPAID: "Төлөөгүй",
  PARTIAL: "Хагас төлсөн",
  PAID: "Төлсөн",
  REFUNDED: "Буцаасан",
};

const PAYMENT_STATUS_CLASSES: Record<string, string> = {
  NOT_REQUIRED: "bg-neutral-100 text-ink-3",
...
const OWNER_STATUS_LABELS: Record<string, string> = {
  ...
  CLAIM_PENDING: "Нэхэмжлэл хүлээж байна",
};

const OWNER_STATUS_CLASSES: Record<string, string> = {
  IDENTIFIED: "bg-green-50 text-green-700",
...
export default async function Page({
  ...
  // Inline server action bound to this parcel
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-3">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-[13px] text-ink font-mono truncate">
              {parcel.publicCode}
            </span>
          </div>
          <div className="flex items-center gap-2.5 flex-wrap">
            <h1 className="text-xl font-black text-ink">
              Бараа #{parcel.publicCode}
            </h1>
            <StatusBadge status={parcel.status} />
            <span
              className={`inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-semibold ${
                OWNER_STATUS_CLASSES[parcel.ownerStatus] ?? "bg-neutral-100 text-ink-3"
              }`}
            >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <dl>
            <InfoRow
              ...
              value={
                parcel.customerId ? (
                  <Link
                    href={`/dashboard/super-admin/customers/${parcel.customerId}`}
            ...
            <InfoRow
              label="Утас"
              value={parcel.customerPhone ?? parcel.customer?.phone ?? null}
            />
            {!parcel.customerId && (
              <InfoRow
                label="Холбоо"
                value={
                  <span className="text-amber-600 text-xs">
                    Системийн хэрэглэгчтэй холбогдоогүй
                  </span>
                }
              />
            )}
        ...
        <SectionCard title="Трак код">
          <dl>
            <InfoRow
              ...
              value={
                parcel.trackCodeOriginal ? (
                  <span className="font-mono text-xs">{parcel.trackCodeOriginal}</span>
                ) : null
              }
            />
            <InfoRow
            ...
            />
            {parcel.cargoType && (
              <InfoRow label="Ачаа төрөл" value={parcel.cargoType} />
            )}
            {parcel.priority && (
              <InfoRow
                label="Яаралтай байдал"
                value={parcel.priority === "URGENT" ? "Яаралтай" : "Энгийн"}
              />
            )}
          </dl>
        </SectionCard>
          ...
          <dl>
            ...
            <InfoRow
              ...
              value={
                <span
                  className={`inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-semibold ${
                    PAYMENT_STATUS_CLASSES[parcel.paymentStatus] ?? "bg-neutral-100 text-ink-3"
                  }`}
                >
        ...
        <SectionCard title="Агуулах / Байршил">
          <dl>
            <InfoRow
              label="Агуулах"
              value={parcel.currentWarehouse?.name ?? null}
            />
            <InfoRow
              label="Бүртгэсэн"
              value={parcel.createdBy?.name ?? null}
            />
            <InfoRow
              label="Огноо"
              value={new Date(parcel.createdAt).toLocaleString("mn-MN")}
            />
          </dl>
        </SectionCard>
        ...
        <div className="p-5">
          <form action={handleUpdateStatus} className="flex items-center gap-3 flex-wrap">
            ...
            <span className="text-xs text-ink-3">
              Одоогийн: <StatusBadge status={parcel.status} />
            </span>
          </form>
        </div>
      </div>
            ...
            {parcel.events.map(
              (event: {
                ...
                <li key={event.id} className="py-3 flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    {event.messageMn && (
                      <p className="text-sm text-ink">{event.messageMn}</p>
                    )}
                    {event.locationText && (
                      <p className="text-xs text-ink-3 mt-0.5">{event.locationText}</p>
                    )}
                  </div>
                  <time className="text-xs text-ink-3 whitespace-nowrap shrink-0">
                    {new Date(event.createdAt).toLocaleString("mn-MN")}
                  </time>
                </li>
              )
            )}
                ...
                {deliveryRequests.map(
                  (dr: {
                    ...
                    >
                      ...
                      <td className="px-5 py-3 text-xs text-ink-3 whitespace-nowrap">
                        {new Date(dr.createdAt).toLocaleString("mn-MN")}
                      </td>
                    </tr>
                  )
                )}
      ...
      {storageRequests.length > 0 && (
        <SectionCard title={`Хадгалалтын хүсэлтүүд (${storageRequests.length})`}>
          <div className="overflow-x-auto -mx-5">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-neutral-100">
              ...
              <tbody>
                {storageRequests.map(
                  (sr: { id: string; status: string; createdAt: Date }) => (
                    <tr
                      key={sr.id}
                      className="border-b border-neutral-100 last:border-0 hover:bg-neutral-50"
                    >
                      ...
                      <td className="px-5 py-3 text-xs text-ink-3 whitespace-nowrap">
                        {new Date(sr.createdAt).toLocaleString("mn-MN")}
                      </td>
                    </tr>
                  )
                )}
