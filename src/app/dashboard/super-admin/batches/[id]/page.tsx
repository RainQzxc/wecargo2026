...
const BATCH_STATUS_VARIANT: Record<
  string,
  "gray" | "blue" | "yellow" | "brand" | "green" | "red" | "purple"
> = {
  DRAFT: "gray",
  LOADING: "yellow",
  LOADED: "yellow",
...
function BatchStatusBadge({ status }: { status: string }) {
  const label =
    BATCH_STATUSES.find((s) => s.value === status)?.label ?? status;
  const cls =
    BATCH_STATUS_CLASSES[status] ?? "bg-neutral-100 text-ink-3";
  return (
    <span
...
export default async function Page({
  ...
  // Server actions bound to this batch
  ...
  async function handleAddParcel(formData: FormData) {
    "use server";
    const publicCode = String(formData.get("publicCode") ?? "").trim();
    if (publicCode) {
      await addParcelToBatch(id, publicCode);
    }
  }
  ...
  async function handleRemoveParcel(formData: FormData) {
    ...
    if (parcelId) {
      await removeParcelFromBatch(id, parcelId);
    }
  }
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-3">
        ...
        <Link
          href="/dashboard/super-admin/batches"
          className="text-sm text-ink-3 hover:text-brand transition-colors whitespace-nowrap"
        >
          &larr; Буцах
        </Link>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <StatCard
          label="Нийт илгээмж"
          value={batchItemsCount}
          color="brand"
        />
        <StatCard
          label="Нийт жин"
          value={totalWeight > 0 ? `${totalWeight.toFixed(2)} кг` : "—"}
          color="default"
        />
        <StatCard
          label="Гарах агуулах"
          value={batch.originWarehouse?.name ?? "—"}
          color="default"
        />
        <StatCard
          label="Очих агуулах"
          value={batch.destinationWarehouse?.name ?? "—"}
          color="default"
        />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <SectionCard title="Бүлгийн мэдээлэл">
          <dl>
            ...
            <InfoRow
              label="Бүртгэсэн"
              value={batch.createdBy?.name ?? null}
            />
          </dl>
        </SectionCard>
        ...
        <SectionCard title="Огноо / Тэмдэглэл">
          <dl>
            <InfoRow
              label="Ачааласан огноо"
              value={
                batch.loadedAt
                  ? new Date(batch.loadedAt).toLocaleString("mn-MN")
                  : null
              }
            />
            <InfoRow
              ...
              value={
                batch.departedAt
                  ? new Date(batch.departedAt).toLocaleString("mn-MN")
                  : null
              }
            />
            <InfoRow
              ...
              value={
                batch.arrivedAt
                  ? new Date(batch.arrivedAt).toLocaleString("mn-MN")
                  : null
              }
            />
            <InfoRow
              ...
              value={
                batch.closedAt
                  ? new Date(batch.closedAt).toLocaleString("mn-MN")
                  : null
              }
            />
        ...
        <div className="p-5">
          ...
          >
            ...
            <span className="text-xs text-ink-3">
              Одоогийн: <BatchStatusBadge status={batch.status} />
            </span>
          </form>
        </div>
      </div>
        ...
        <div className="p-5">
          ...
          >
            <input
              name="publicCode"
              type="text"
              placeholder="Нийтийн код оруулна уу"
        ...
        {batchItemsCount === 0 ? (
          <div className="px-5 py-12 text-center">
            <p className="text-sm text-ink-3">
              Энэ бүлэгт илгээмж байхгүй байна.
            </p>
            <p className="text-xs text-ink-3 mt-1">
              Дээрх маягтаар нийтийн код ашиглан илгээмж нэмнэ үү.
            </p>
          </div>
        ) : (
                      ...
                      <td className="px-5 py-3">
                        {parcel ? (
                          <Link
                            ...
                            className="font-mono text-brand hover:underline text-xs"
                          >
                            {parcel.publicCode}
                          </Link>
                        ) : (
